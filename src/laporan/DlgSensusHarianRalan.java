...
//                    dx2=Sequel.cariIsi("select kd_penyakit from diagnosa_pasien where prioritas='2' and status='Ralan' and no_rawat=?",rs.getString("no_rawat"));
//                    dx3=Sequel.cariIsi("select kd_penyakit from diagnosa_pasien where prioritas='3' and status='Ralan' and no_rawat=?",rs.getString("no_rawat"));
//                    dx4=Sequel.cariIsi("select kd_penyakit from diagnosa_pasien where prioritas='4' and status='Ralan' and no_rawat=?",rs.getString("no_rawat"));
//                    dx5=Sequel.cariIsi("select kd_penyakit from diagnosa_pasien where prioritas='5' and status='Ralan' and no_rawat=?",rs.getString("no_rawat"));
//                    
                    diagnosasekunder="";
                    //if(rs2.getRow()>0){
                    ps5=koneksi.prepareStatement(
                            "select kd_penyakit from diagnosa_pasien "+
                            "where diagnosa_pasien.no_rawat=? and prioritas<>'1' order by prioritas asc");
                    try {                                    
                        ps5.setString(1,rs.getString("no_rawat"));
                        rs5=ps5.executeQuery();
                        while(rs5.next()){
                        if(diagnosasekunder.equals("")){
                                        diagnosasekunder=rs5.getString(1);
//                                        diagnosasekunder=rs5.getString(2);
                                    }else{
                                        diagnosasekunder=diagnosasekunder+", "+rs5.getString(1);
                                    }
                                }
//                        
                    } catch (Exception e) {
                    ...
                    //if(rs2.getRow()>0){
                    ps5=koneksi.prepareStatement(
                            "select nm_penyakit from penyakit INNER JOIN diagnosa_pasien "+
                            "on diagnosa_pasien.kd_penyakit=penyakit.kd_penyakit "+
                            "WHERE diagnosa_pasien.no_rawat=? and prioritas<>'1'");
                    try {                                    
                        ps5.setString(1,rs.getString("no_rawat"));
                        rs5=ps5.executeQuery();
                        while(rs5.next()){
                        if(penyakitsekunder.equals("")){
                                        penyakitsekunder=rs5.getString(1);
//                                        diagnosasekunder=rs5.getString(2);
                                    }else{
                                        penyakitsekunder=penyakitsekunder+", "+rs5.getString(1);
                                    }
                                }
//                        
                    } catch (Exception e) {
